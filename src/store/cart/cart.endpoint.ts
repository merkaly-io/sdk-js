...
namespace Cart {
  export const find = (params?: FindCartValidator) => {
    return MerkalySDK.prototype.$axios.$get<CartReference[]>('/store/carts', { params })
      .then(carts => carts.map(cart => plainToInstance(FindCartValidator, cart)))
  }

  export const read = (id: IdCartValidator) => {
    return MerkalySDK.prototype.$axios.$get<CartReference>('/store/carts' + id)
      .then(cart => plainToInstance(FindCartValidator, cart))
  }

  export const create = (payload: CreateCartValidator) => {
    return MerkalySDK.prototype.$axios.$post<CartReference>('/store/carts', payload)
      .then(cart => plainToInstance(FindCartValidator, cart))
  }

  export const update = (id: IdCartValidator, payload: UpdateCartValidator) => {
    return MerkalySDK.prototype.$axios.$patch<CartReference>('/store/carts' + id, payload)
      .then(cart => plainToInstance(FindCartValidator, cart))
  }

  export const remove = (id: IdCartValidator) => {
    return MerkalySDK.prototype.$axios.$delete<void>('/store/carts' + id)
  }
}
